...
export default function DesktopTrotHub({ ideas }: { ideas: Idea[] }) {
  const trottedIdeas = ideas.filter(i => i.status === 'trotted').sort((a, b) => (b.trots || 0) - (a.trots || 0));
                ...
                {trottedIdeas.map((idea, i) => (
                  <tr key={idea.id} className="border-b border-border-light/50 hover:bg-bg-darker transition-colors">
                    ...
                    <td className="py-4 px-4">
                      ...
                      <div className="text-[10px] text-zinc-500 font-mono mt-1">{idea.trots || 0} TROTS</div>
                    </td>
                    ...
                    <td className="py-4 px-4 text-right">
                      <button className="brutalist-button-green px-4 py-2 text-xs">Claim to Build 🐎</button>
                    </td>
                  </tr>
                ))}
          ...
          <div className="w-80 space-y-8">
            <div className="border border-toxic-purple p-6 bg-bg-card shadow-[8px_8px_0_0_#A855F7]">
              ...
              <div className="pt-4 border-t border-border-light">
                 ...
                 <div className="flex gap-2">
                   <button className="border border-border-light px-3 hover:bg-bg-darker">-</button>
                   <input type="text" value="$TROT 25" readOnly className="w-full bg-bg-darker border border-border-light text-center font-mono text-sm text-white" />
                   <button className="border border-border-light px-3 hover:bg-bg-darker">+</button>
                 </div>
                 <button className="w-full brutalist-button-green py-3 text-xs mt-4">PLEDGE NOW ⚡</button>
              </div>
            </div>
            ...
            <div className="border border-hyper-green p-6 bg-bg-card">
              ...
              <div className="space-y-4 text-[10px] font-mono">
                 <div className="flex justify-between text-zinc-400"><span>00:00:01.5</span><span className="text-hyper-green">TROT</span><span>Phantom Forge</span></div>
                 <div className="flex justify-between text-zinc-400"><span>00:00:02.9</span><span className="text-toxic-purple">ROT</span><span>Mood Forge</span></div>
                 <div className="flex justify-between text-zinc-400"><span>00:00:04.1</span><span className="text-hyper-green">TROT</span><span>Decay Relay</span></div>
              </div>
            </div>
          </div>
        </div>
